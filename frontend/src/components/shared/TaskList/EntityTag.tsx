...
export const EntityTag = ({ task }: EntityTagProps) => {
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (isClient && task.linked_entity_id) {
      ...
      // Navigate to client detail with applications tab and visa application ID
      if (clientId) {
        navigate(`/clients/${clientId}?tab=applications&visaApplicationId=${task.linked_entity_id}`);
      } else if (!loading) {
        // If we couldn't fetch client_id, still try to navigate
        navigate(`/clients/${task.linked_entity_id}?tab=applications&visaApplicationId=${task.linked_entity_id}`);
      }
    }
  };
